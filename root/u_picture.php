         ...
         <div class="container">
            <div class="row">
               <div class="col-md-6">   
					...
					<h1><font color="#ffffff">Select image to upload:</h1></font>
               <br>
					<input type="file" name="image">
					<Button value="Upload" name="Upload" style="height:45px; width:120px; margin-left:0px;"/><font size="4"> Upload</font> </button>
					</form>
           <?php
/*		$cn=mysqli_connect("localhost","root","");
		...
		while($row=mysqli_fetch_array($q))
		{
         ...
         src="data:image/jpeg;base64,'.base64_encode( $row[1] ).'"/></a>&nbsp&nbsp&nbsp';
		}
*/
	if(isset($_POST["Upload"]))
	{
      ...
      $name=addslashes($_FILES["image"]["name"]);
		...
		$cn=mysqli_connect("localhost","root","");
		$db=mysqli_select_db($cn,"project");
		$q=mysqli_query($cn,"insert into image values('".NULL."','".$name."','".$img."')");
			if($q)
			{
            echo "<script type='text/javascript'>alert('$message');</script>";
         header("location:upload.php?id=1");
			}
			else
			{
